...
def create_session_api(identifier: str, password: str) -> tuple[str, str]:
    ...
    resp.raise_for_status()
    data = resp.json()
    # The session response contains both an access JWT for authentication and
    # the fully-qualified DID associated with the handle.  Returning both
    # values allows downstream API calls to use the correct actor DID (which
    # may differ from the DID returned by resolve_handle_api).
