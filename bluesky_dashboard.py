...
if (follower_csv and engagement_csv) or (
    ...
    account_creation_default = pd.to_datetime("2025-07-26")
    account_creation_date = st.sidebar.date_input(
        "Account creation date", value=account_creation_default, min_value=followers_df['date'].min(), max_value=followers_df['date'].max()
    )
    # Determine the available date range and let the user choose a subset
    min_date = max(account_creation_date, min(followers_df['date'].min(), engagement_df['date'].min()))
    ...
    start_filter = max(account_creation_date, start_date)
    ...
    # Generate a heatmap using Plotly.  Use a perceptually uniform colour
    # scale (YlOrRd) to represent intensity.  Display values on hover.
    fig = px.imshow(
        pivot_table,
